...
In this way, we have found a $95 \%$ confidence interval for $\exp(\beta_{\text{height}})$. This means that we can say with $95\%$ confidence that an increase in height of 1 meter means that the probability of a successful ascent will be multiplied by $\exp(\beta_{\text{height}}) \in \big[0.9981,0.9986 \big]$.
...
We see some indication of a larger spread of the residuals towards higher probabilities and some non-centering around lower probabilites, but observe no obvious structure given the amount of data used. 

```{r}
ggplot(df, aes(x = height, y = dres, size = observations)) + geom_point()
```

The $x$-axis now indicates height. Again we see a larger spread at at lower altitudes, but also that the covariate patterns at lower altitudes have a smaller amount of observations. Beyond that we see no apparent structure in the data. 
...
```{r, eval=T, echo=T}
abs(fit$coefficients['height'] / fit$coefficients['prominence'])
```



Plotting the predicted probabilites as a function of `height` and `prominence` gives us 
...
$$
\hat{\beta}_0 + x_{i1} \hat{\beta}_1 + x_{i2} \hat{\beta}_2 = x_i^T \hat{\boldsymbol \beta} \sim N(x_i^T \beta, x_i F^{-1}(\hat{\boldsymbol \beta})x_i^T).
$$
Which we can use to calculate the confidence intervals for the linear predictor $\eta_i$. We can then transform the linear predictor to the corresponding probabilites and get a confidence interval for ascent success probability for Mount Everest and Chogolisia. 
...
eta_limits_ME = c(eta_ME + qnorm(0.025) * std_ME, eta_ME + qnorm(0.975) * std_ME)
eta_limits_CH = c(eta_CH + qnorm(0.025) * std_CH, eta_CH + qnorm(0.975) * std_CH)

limits = rbind(eta_limits_ME, eta_limits_CH)
rownames(limits) = c('Mt. Everest', 'Chogolisia')
colnames(limits) = c('2.5%', '97.5%')
```
...
, as we did when we made a confidence interval for $\beta_h$ in part 1b). In the same way as we did in part 1b), we get the coefficient estimates and the estimated standard deviations from the `r` printout, from which we can construct a confidence interval for the linear predictor $LP = \beta_I + \beta_hx_{ME,h} + \beta_px_{ME,p}$. We denote by $\widehat{LP} = \hat{\beta}_I+\hat{\beta}_h\cdot x_{ME,h} + \hat{\beta}_p \cdot x_{ME,p}$ the estimated linear predictor, 

and $\widehat{sd(LP)} = \sqrt{c_{00}+c_{11}\cdot x_{ME,h}^2+c_{22}\cdot x_{ME,p}^2}$ the standard deviation for the estimated linear predictor.

$\hat{\beta_i}$ ER IKKE NØDVENDIGVIS UAVHENGIGE OG VI MÅ DERFOR TA MED KOVARIANSLEDDENE, eller?
