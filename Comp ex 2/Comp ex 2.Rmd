...
As mentioned in the Wald-test. The parameteres are asymptotically normal with mean $\beta$ and variance $c_{jj}$. The `r` printout gives us the coefficient estimates and the estimated standard deviation $\sqrt{c_{jj}}$, of parameter $j$. From this which we can construct a confidence interval. We can make a $95 \%$ confidence interval for the height parameter $\beta_{\text{height}}$. We have $95\%$ confidence that
...
In this way, we have found a $95 \%$ confidence interval for $\exp(\beta_{\text{height}})$. This means that we can say with $95\%$ confidence that an increase in height of 1 meter means that the odds will be multiplied by $\exp(\beta_{\text{height}}) \in \big[0.9981,0.9986 \big]$.
...
$$\widehat{LP}=\hat{\beta}_0 + x_{i1} \hat{\beta}_1 + x_{i2} \hat{\beta}_2 = x_i^T \hat{\boldsymbol \beta} \sim N_1(x_i^T \beta, x_i F^{-1}(\hat{\boldsymbol \beta})x_i^T).$$
