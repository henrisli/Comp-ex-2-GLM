...
To express the log likelihood as a function of $\boldsymbol \beta$ we first use the link between $\eta_i$ and $\pi_i$. We use the response function $h(\eta) = g^{-1}(\eta)$, which isthe inverse of the link function. $\pi_i = h(\eta_i) = \frac{\exp(\eta_i)}{1+\exp(\eta_i)}$. The log-likelihood function can then be written
...
$$
s(\boldsymbol \beta) = \frac{\partial l(\boldsymbol \beta)}{\partial \beta} = \sum_{i=1}^n y_i x_i - n_i \frac{x_i \exp{\mathbf{x}_i^T \boldsymbol \beta}}{1 + \exp{\mathbf{x}_i^T \boldsymbol \beta}}.
$$
...
$$
s(\boldsymbol \beta) = \frac{\partial l (\boldsymbol \beta)}{\partial \boldsymbol \beta} = \boldsymbol 0.
$$
Which can be found iteratively by letting $n \rightarrow \infty$
$$
\boldsymbol \beta_{n+1} = \boldsymbol \beta_n + F^{-1}(\boldsymbol \beta_n) s(\boldsymbol \beta_n)
$$
...
We denote $c_jj$ for diagonal element $j$ of $F^{-1}(\hat{\boldsymbol \beta})$ and get that for each coefficient $\beta_j$,
...
$$
\hat{\beta}_0 + x_{i1} \hat{\beta}_1 + x_{i2} \hat{\beta}_2 = x_i^T \hat{\boldsymbol \beta} \sim N_1(x_i^T \beta, x_i F^{-1}(\hat{\boldsymbol \beta})x_i^T).
$$
...
<!-- Again, we denote $c_{jj}$ for diagonal element $j$ of $F^{-1}(\hat{\boldsymbol \beta})$, so $c_jj$ will be the variance of $\hat{\beta}_j$. Then, we get that -->
...
<!-- Mount Everest has prominence of 8848 and height of 8848, which are both outside the data range used to fit the model. We therefore have no way to control if the model is valid for this range, and can therefore not argue that the predicted probabilites are a good approximation. Therefore, it is not reasonable to estimate the probability of climbing Mount Everest with this model. In addition, the confidence interval is really large (almost the whole range of the response [0,1]), and thus quite useless. **SKRIV KANSKJE LITT MER HER** -->
...
<!-- Finally we calculate a $95\%$ confidence interval for the probability, which gives -->
...
**SPØR INGEBORG OM DET ER 20 ELLER 4 SUCCESS FOR CHOGOLISA**
