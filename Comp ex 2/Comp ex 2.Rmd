...
We have made the following assumptions for the model:

1. $Y_i$ \tilde bin($n_i,\pi_i$), and E($Y_i$) = $\mu_i$ = $\pi_i$, and Var($Y_i$) = \pi_i(1-\pi_i).
2. Linear predictior: $\eta_i = x_i^T\beta$.
3. Logit link: $\eta_i = \text{ln}\big(\frac{\pi_i}{1-\pi_i}\big)$

where **$x_i$** is the p dimentional column vector of covariates for observation i and **$\beta$** is the vector of regression parameters.
...
\begin{equation}
l(\beta) = \sum_{i=1}^n [y_i \text{ln}(\pi_i)+(1-y_i)\text{ln}(1-\pi_i)] 
 = \sum_{i=1}^n [y_i \text{ln}\big(\frac{\pi_i}{1-\pi_i}\big)+\text{ln}(1-\pi_i)]
\end{equation}

To express the log likelihood as a function of $\beta$ we use the link between $\eta_i$ and $\pi_i$. We have that $\pi_i = \frac{e^{\eta_i}}{1+e^{\eta_i}}$. The log likelihood function can then be written:


\begin{equation}
l(\beta) = \sum_{i=1}^n[y_i\eta_i+\text{ln}(\frac{1}{1+\text{exp}(\eta_i)})]
=  \sum_{i=1}^n[y_i\eta_i-\text{ln}(1+\text{exp}(\eta_i)).
\end{equation}


Now we use that $\eta_i = x_i^T\beta$ and get:

\begin{equation}
l(\beta)=\sum_{i=1}^n[y_ix_i^T \beta - \text{ln} (1+\exp(x_i^T\beta))].
\end{equation}

which is an expression of the log likelihood as a function of beta.

Since we know that this function is concave we can find the parameters, $\beta$ that gives the highest likelihood by taking the partial derivatives with respect to $\beta$ to get the score function, $s$. The parameters $\beta$ that gives the highest likelihood is those for which the score function equal to zero.
...
\begin{equation}
\pi_i = \frac{\text{exp}^{\eta_i}}{1+\text{exp}^{\eta_i}}.
\end{equation}

This can be rewritten:

\begin{equation}
\eta_i = \text{ln}\big(\frac{\pi_i}{1-\pi_i}\big).
\end{equation}

Our linear predictor is $\eta_i = x_i^T\beta$. If we insert this we get:
$$
\ln\big(\frac{\pi_i}{1-\pi_i}\big) = \beta_0 + \beta_1x_{i1} + \beta_2x_{i2} + ... + \beta_kx_{ik}
$$
and
$$
\frac{\pi_i}{1-\pi_i} = \exp(\beta_0) \cdot \exp(\beta_1x_{i1})\cdot \exp(\beta_2x_{i2})\cdot\cdot\cdot\exp(\beta_kx_{ik}).
$$

This shows that a one increase in hight will give a increase in the odds equal to $\exp(\beta_1)$.
