...
1. Model for response: $Y_i \sim \text{Bin}(n_i,\pi_i)$ for $i = 1,\dots,113$, with $\text{E}(Y_i) = \mu_i = n_i\pi_i$ and $\text{Var}(Y_i) = n_i\pi_i(1-\pi_i)$
...
$$L(\boldsymbol \beta)= \prod_{i=1}^G L_i(\boldsymbol\beta) = \prod_{i=1}^G f(y_i;\boldsymbol\beta) = \prod_{i=1}^G{n_i \choose y_i} \pi_i^{y_i}(1-\pi_i)^{n_i-y_i}.$$

If we take the natural logarithm, we get the log likelihood function

$$
l(\boldsymbol \beta) = \ln(L(\boldsymbol \beta)) = \sum_{i=1}^G l_i(\boldsymbol \beta) \\= \sum_{i=1}^G [y_i \ln(\pi_i)+(n_i-y_i)\ln(1-\pi_i) + \ln{n_i \choose y_i}]
 = \sum_{i=1}^G [\ln{n_i \choose y_i} + y_i \ln\big(\frac{\pi_i}{1-\pi_i}\big)+n_i \ln(1-\pi_i)]
$$

To express the log likelihood as a function of $\boldsymbol \beta$ we first use the link between $\eta_i$ and $\pi_i$. We use the response function $h(\eta) = g^{-1}(\eta)$, which is the inverse of the link function. $\pi_i = h(\eta_i) = \frac{\exp(\eta_i)}{1+\exp(\eta_i)}$. The log-likelihood function can then be written:

$$
l(\boldsymbol \beta) = \sum_{i=1}^G[\ln{n_i \choose y_i} + y_i\eta_i+n_i \ln\big(\frac{1}{1+\exp(\eta_i)}\big)]
= \sum_{i=1}^G[\ln{n_i \choose y_i} + y_i\eta_i-n_i \ln(1+\exp(\eta_i))].
$$

Finally we use that $\eta_i = {\bf x}_i^T \boldsymbol \beta$ and get

$$
l(\boldsymbol \beta)=\sum_{i=1}^G[\ln{n_i \choose y_i} + y_i {\bf x}_i^T \boldsymbol \beta - n_i \ln(1+\exp({\bf x}_i^T \boldsymbol \beta)],
$$
...
$$
\boldsymbol s(\boldsymbol \beta) = \frac{\partial l(\boldsymbol \beta)}{\partial \boldsymbol\beta} = \sum_{i=1}^G \big[y_i x_i - n_i \frac{x_i \exp{\mathbf{x}_i^T \boldsymbol \beta}}{1 + \exp{\mathbf{x}_i^T \boldsymbol \beta}}\big].
$$
...
$$
\boldsymbol s(\boldsymbol \beta) = \frac{\partial l (\boldsymbol \beta)}{\partial \boldsymbol \beta} = \boldsymbol 0.
$$
In practice, there is often no closed form solution, which means that we have to use numerical procedures to fin the betas, such as Newton-Raphson or Fisher Scoring. The betas can be found iteratively by the formula
$$\boldsymbol \beta_{n+1} = \boldsymbol \beta_n + F^{-1}(\boldsymbol \beta_n) \boldsymbol s(\boldsymbol \beta_n),$$
where $F^{-1}(\beta_n)$ is the inverse of the expected Fisher information.

## b)
We load the dataset and fit a model as described in Part a) with success rate
as response and height and prominence as predictors. 
...
We denote $c_{jj}$ for diagonal element $j$ of $F^{-1}(\hat{\boldsymbol \beta})$ and get that for each coefficient $\beta_j$,
$$\frac{\beta_j - \hat{\beta_j}}{\sqrt{c_{jj}}} \sim N(0,1).$$
We can test the significance of the $\beta_j$ using the test statistics 
...
Because the p-value is below any reasonable signifiance level, we reject the null-hypothesis. So our candidate model does not fit the data well. Although all the parameters are significant, this test indicates that we haven't included enough parameters to make a good model. The model could be improved if we included additional covariates, e.g. weather, amount of avalanches, "fitness" of climber, etc.

As mentioned in the previous paragraph about the Wald-test, the parameters are each asymptotically normal with mean $\beta$ and variance $c_{jj}$. The `r` printout gives us the coefficient estimates and the estimated standard deviation $\sqrt{c_{jj}}$ of parameter $j$. From this we can construct a confidence interval. We can make a $95 \%$ confidence interval for the height parameter $\beta_{\text{height}}$ in the following way
...
In this way, we have found a $95 \%$ confidence interval for $\exp(\beta_{\text{height}})$. This means that we can say with $95\%$ confidence that an increase in height of 1 meter means that the odds will be multiplied by $\exp(\beta_{\text{height}}) \in \big[0.9981,0.9986 \big]$. This is intuitive, because a higher mountain should in theory be harder to climb.
...
df = data.frame(fitted = fit$fitted.values, dres = residual, height = mount$height, prominence = mount$prominence, success = (fit$data['success']), fail = (fit$data['fail']))
...
$$ \frac{\hat{\beta}_{height}}{\hat{\beta}_{prominence}} = 9.4$$
...
Based on the model we have fitted, we want to estimate the probability of successfully ascending Mount Everest (height and prominence both equal 8848m), and Chogolisa (height 7665m and prominence 1624m). We denote Mt. Everest for $x_1$ and Chogolisa for $x_2$. We use our fitted model to produce linear predictors $\eta_i$ for both summits. 

```{r, eval=T, echo=F}
limits[,2]
```
And we transform the linear predictors to probabilites using the response function $h(\eta) = \exp(\eta) / (1 + \exp(\eta))$,
```{r, echo=F, eval=T}
exp(limits[,2])/(1+ exp(limits[,2]))
```

We also want to make a $95 \%$ confidence interval for the probability of successfully ascending both summits. We use the fact that the coefficients found by maximum likelihood asymptotically will be normal distributed with mean $\boldsymbol \beta$ and variance $F^{-1}(\hat{\boldsymbol \beta})$, $\hat{\boldsymbol \beta} \sim N(\boldsymbol \beta, F^{-1}(\hat{\boldsymbol \beta}))$. We then get that the linear predictor, a linear combination of the parameters $\beta_j$, is also normal distributed,
$$\widehat{LP}=\hat{\beta}_0 + x_{i1} \hat{\beta}_1 + x_{i2} \hat{\beta}_2 = x_i^T \hat{\boldsymbol \beta} \sim N_1(x_i^T \beta, x_i^T F^{-1}(\hat{\boldsymbol \beta})x_i).$$
Which we can use to calculate the confidence intervals for the linear predictor $\eta_i$. We can then transform the linear predictor CI to the corresponding probability CI and get an interval for ascent success probability for Mount Everest and Chogolisa.
As the estimated linear predictor is normally distributed, we get that
...
where $\widehat{LP} = \hat{\beta}_I+\hat{\beta}_h\cdot x_{ME,h} + \hat{\beta}_p \cdot x_{ME,p}$ and $\widehat{sd(LP)} = \sqrt{x_i^TF^{-1}(\hat{\boldsymbol \beta})x_i}$.

For the linear predictor we get limits
```{r, echo=F, eval=T}
limits
```
Which is transformed to the probabilites
```{r, echo=F, eval=T}
...
$$H_0: \text{The home and away goals are independent},\quad H_1: \text{They are not independent}.$$
